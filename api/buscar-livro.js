...
module.exports = async function handler(req, res) {
  ...
  // Tenta algumas estratégias de busca em sequência, e entre os resultados de cada uma,
  // usa o primeiro que realmente tiver uma capa cadastrada (nem todo resultado tem).
  let capaUrl = '';
  ...
  for (const query of tentativas) {
    if (capaUrl) break;
    try {
      const booksResp = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${query}&maxResults=5`);
      if (!booksResp.ok) continue;
      ...
      // Segue tentando a próxima estratégia.
    }
  }
